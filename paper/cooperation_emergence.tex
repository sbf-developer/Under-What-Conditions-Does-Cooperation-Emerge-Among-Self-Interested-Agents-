...
\subsection{Plain-language takeaway}

Cooperation among self-interested agents is \textbf{not mysterious}, but it is \textbf{not automatic}. It emerges when the shadow of the future is long enough, when cooperators can find each other (genetically, spatially, or socially), when errors are small enough to preserve reciprocity, or when communities build rules that reward contribution and penalize free-riding. Where those conditions fail---one-shot anonymous interactions with high temptation to defect---self-interest predicts the tragedy of the commons \citep{hardin1968}.
